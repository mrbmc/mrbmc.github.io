### Recent Highlights{.center #highlights}
+ Launched [Growth Design for Disney+](/portfolio/disney-growth/) & ESPN+ propelling them from **0-126M subscribers in two years**.
+ Scaled a design team at MLB from 2 - 26 people including UX Research, Growth, Sports, and Enterprise SAAS.
+ Co-authored the designer career framework, hiring playbook, and performance tools for Disney leading to a 30% reduction in attrition, and 3x decrease in hiring time.
+ [Led product design for STAR+](https://medium.com/disney-streaming/star-from-the-ground-up-building-a-brand-new-streaming-service-for-latin-america-fbe9f5b9366a), proving that a single streaming app could unify general entertainment and live sports. That product became the template for Disney's aggregation strategy.
+ Led the design of [Disney+ GroupWatch](/portfolio/disney-groupwatch/), a social co-viewing feature that reached 15% adoption and set a new model for how Disney ships consumer innovation.
+ Built & managed a program for [SAAS Tools at Disney](/portfolio/disney-enterprise/), yielding $100M/pa cost savings and 200% growth in AARPU.
+ Led the design of streaming services for NHL, MLB, Eurosport, PGA, and MLS - growing MLB's streaming business toward a $4B exit.
+ Led the team building a [patented](https://patents.google.com/patent/US20170195373A1/) podcast platform that paid creators a fair commission at [BlogTalkRadio](/portfolio/blogtalkradio/) – leading to a $55M acquisition.  
  
  [Resume (2 page PDF)](/images/uploads/Brian_McConnell-Resume_2026.pdf){.button}

{.card}
